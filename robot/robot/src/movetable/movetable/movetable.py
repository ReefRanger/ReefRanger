...
class navigatetable(Node):

    def __init__(self):
        super().__init__('navigate_table')
    ...
    def execute_callback(self, goal_handle):
        ...
        self.target_x = goal_handle.request.targetx
        self.target_y = goal_handle.request.targety
        self.target_z = -2.8
        ...
        try:
            while rclpy.ok():
                ...
                if (
                    abs(self.target_x - self.x) < 0.3 and
                    abs(self.target_y - self.y) < 0.3 and
                    abs(self.target_z - self.z) < 0.3
                ):
